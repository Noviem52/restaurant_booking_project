...
export default function BookingWidget({
    ...
    return (
        <div className="bg-surface-container-lowest border border-outline-variant/20 p-6 rounded-md shadow-sm text-left">
            ...
            <div className="space-y-4">
                ...
                <div className="space-y-1">
                    <label className="block text-[10px] font-medium text-on-surface/55 tracking-wider uppercase">PARTY SIZE</label>
                    <div className="relative">
                        <Users className="absolute left-3 top-3 text-on-surface/55" size={16} />
                        <select
                ...
                {}
                <div className="space-y-1">
                    <label className="block text-[10px] font-medium text-on-surface/55 tracking-wider uppercase">DATE</label>
                    <div className="relative">
                        <Calendar className="absolute left-3 top-3 text-on-surface/55" size={16} />
                        <input
                            type="date"
                            value={selectedDate}
                ...
                <div className="space-y-2 pt-2">
                    <span className="block text-[10px] font-medium text-on-surface/55 tracking-wider uppercase">AVAILABLE TIMES</span>
                            ...
                            (() => {
                                ...
                                const allSlots =
                                    slotsAvailability.length > 0
                                        ? slotsAvailability
                                        : (cafe.availableSlots || []).map((s: string) => ({
                                              time: s,
                                              availableSeats: 20,
                                              isAvailable: true,
                                          }));
                                return allSlots.filter((slotInfo: SlotAvailability) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`py-2 px-1 text-center text-[10px] font-medium tracking-wider uppercase border transition-all rounded-sm ${
                                            isSelected
                                                ? "bg-secondary border-secondary text-on-secondary shadow-sm cursor-pointer"
                                                : isFull
                                                  ? "bg-on-surface/5 border-outline-variant/10 text-on-surface/25 cursor-not-allowed opacity-50"
                                                  : "border-outline-variant/40 text-on-surface/55 hover:border-primary hover:text-primary cursor-pointer"
                                        }`}
                                    >
                                        {slot}
                ...
                <p className="text-center text-[10px] text-on-surface/55 mt-3 leading-relaxed">

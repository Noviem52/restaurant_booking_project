import { Link } from "react-router-dom";
import { bottomLinks, footerSections, socialLinks } from "../assets/assets";
import { useAppContext } from "../context/AppContext";
import Logo from "./Logo";

export default function Footer() {
    const { darkMode } = useAppContext();

    return (
        <footer className="w-full bg-surface-container-low border-t border-outline-variant/20 pt-20 pb-12">
            <div className="max-w-7xl mx-auto px-6 md:px-10 grid grid-cols-1 md:grid-cols-4 gap-10">
                {}
                <div className="md:col-span-1">
                    <Link to="/" className="text-primary block mb-2">
                        <Logo
                            bold={darkMode}
                            className={`h-10 w-auto mt-1.5 ${
                                darkMode ? "text-white" : "text-primary"
                            }`}
                        />
                    </Link>

                    <p className="text-on-surface/55 text-sm leading-relaxed pr-4">
                        Discover exceptional cafés, handcrafted coffee, cozy
                        spaces, and memorable moments in every neighborhood.
                    </p>
                </div>

                {}
                {footerSections.map((section) => (
                    <div
                        key={section.title}
                        className="flex flex-col gap-4"
                    >
                        <h4 className="text-xs font-medium tracking-widest text-primary uppercase">
                            {section.title}
                        </h4>

                        {section.links.map((link) => (
                            <Link
                                key={link.label}
                                to={link.path}
                                className="text-sm text-on-surface/55 hover:text-secondary transition-colors"
                            >
                                {link.label}
                            </Link>
                        ))}
                    </div>
                ))}

                {}
                <div className="flex flex-col gap-4">
                    <h4 className="text-xs font-medium tracking-widest text-primary uppercase">
                        CAFÉ SUPPORT
                    </h4>

                    <p className="text-sm text-on-surface/55">
                        customerservice@cafecircle.com
                    </p>

                    <p className="text-sm text-on-surface/55 leading-relaxed">
                        Need help finding the perfect coffee spot? Our café
                        support team is here to help.
                    </p>

                    <div className="flex gap-4 mt-2">
                        {socialLinks.map(({ icon: Icon, href }, index) => (
                            <a
                                key={index}
                                href={href}
                                aria-label="Visit Café Circle social media"
                                className="text-on-surface/55 hover:text-secondary transition-colors"
                            >
                                <Icon size={18} />
                            </a>
                        ))}
                    </div>
                </div>
            </div>

            {}
            <div className="max-w-7xl mx-auto px-6 md:px-10 mt-16 pt-8 border-t border-outline-variant/10 flex flex-col md:flex-row justify-between items-center gap-4">
                <p className="text-xs text-on-surface/55">
                    © 2026 Café Circle. All rights reserved.
                </p>

                <div className="flex gap-6">
                    {bottomLinks.map((link) => (
                        <Link
                            key={link.label}
                            to={link.path}
                            className="text-xs text-on-surface/55 hover:text-secondary transition-colors"
                        >
                            {link.label}
                        </Link>
                    ))}
                </div>
            </div>
        </footer>
    );
}
